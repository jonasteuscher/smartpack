...
const GtcPage = () => {
  ...
  const hero = t('gtcPage.hero', { returnObjects: true }) as TermsHero;
  const summary = t('gtcPage.summary', { returnObjects: true }) as TermsSummary;
  const sections = t('gtcPage.sections', { returnObjects: true }) as TermsSection[];
  const contact = t('gtcPage.contact', { returnObjects: true }) as TermsContact;
          ...
          <div className="grid gap-8 lg:grid-cols-2">
            {sections.map((section) => (
              ...
              >
                ...
                {section.note && <p className="mt-6 text-xs text-slate-500 dark:text-slate-400">{section.note}</p>}
              </article>
            ))}
          </div>
        </div>
      ...
      <section>
        <div className="container-responsive">
          <div className="rounded-3xl border border-slate-100 bg-white p-8 shadow-sm dark:border-slate-800/60 dark:bg-slate-900/70">
            ...
            <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">{contact.description}</p>
            <div className="mt-6 grid gap-6 md:grid-cols-2">
              <div>
                ...
                <a
                  ...
                  className="mt-3 inline-flex items-center gap-2 rounded-full bg-slate-900 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-white shadow transition hover:-translate-y-0.5 hover:bg-brand-secondary dark:bg-brand-secondary"
                >
                  {contact.email}
                </a>
              </div>
              <div>
                ...
                <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">{contact.address}</p>
              </div>
            </div>
          </div>
        </div>
